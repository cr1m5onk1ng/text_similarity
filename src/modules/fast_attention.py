...
class FastSelfAttention(nn.Module):
    def __init__(self, dim, causal = False, heads = 8, local_heads = 0, local_window_size = 256, nb_features = None, feature_redraw_interval = 1000, generalized_attention = False, kernel_fn = nn.ReLU(), qr_uniform_q = False, dropout = 0.):
        ...
        self.fast_attention = FastAttention(self.attention_head_size, nb_features, feature_redraw_interval, causal = causal, generalized_attention = generalized_attention, kernel_fn = kernel_fn, qr_uniform_q = qr_uniform_q)
    ...
    def forward(self, x, context = None, mask = None, context_mask = None, output_attentions=False):
        b, n, _, h, gh = *x.shape, self.num_attention_heads, self.global_heads
        
        cross_attend = exists(context)
        context = default(context, x)
        context_mask = default(context_mask, mask).long()
        ...
        attn_outs = []
        #print(f"context mask type: {context_mask.dtype}")
        if not empty(q):
        ...
        out_attn = torch.cat(attn_outs, dim = 1)
        out = rearrange(out_attn, 'b h n d -> b n (h d)')
        out =  self.to_out(out)
        out = self.dropout(out)
        if output_attentions:
            return (out, tuple(attn_outs))
        return (out,)
    ...
    def __init__(self, dim, depth, heads, local_attn_heads = 0, local_window_size = 256, causal = False, ff_mult = 4, nb_features = None, reversible = False, ff_chunks = 1, generalized_attention = False, kernel_fn = nn.ReLU(), qr_uniform_q = False, use_scalenorm = False, use_rezero = False, ff_glu = False, ff_dropout = 0., attn_dropout = 0., cross_attend = False):
        ...
        for _, local_heads in zip(range(depth), local_attn_heads):
            layers.append(nn.ModuleList([
                wrapper_fn(FastSelfAttention(dim, causal = causal, heads = heads, local_heads = local_heads, local_window_size = local_window_size, nb_features = nb_features, generalized_attention = generalized_attention, kernel_fn = kernel_fn, qr_uniform_q = qr_uniform_q, dropout = attn_dropout)),
            ...
            layers.append(nn.ModuleList([
                wrapper_fn(FastSelfAttention(dim, heads = heads, nb_features = nb_features, generalized_attention = generalized_attention, kernel_fn = kernel_fn, qr_uniform_q = qr_uniform_q, dropout = attn_dropout)),
                wrapper_fn(Chunk(ff_chunks, FeedForward(dim, mult = ff_mult, dropout = ff_dropout, glu = ff_glu), along_dim = 1))
            ]))
